...
class Workout(models.Model):
    ...
    def create(self, vals):
        no = self.env['ir.sequence'].next_by_code('gym.workout')
        vals.update({
            'no': no
        })
        result = super(Workout, self).create(vals)
        return result
    @api.constrains('from_date','to_date')
    def _check_from_date(self):
        date_obj = self.env['gym.day']
        self.from_date_obj = datetime.datetime.strptime(self.from_date, DEFAULT_SERVER_DATETIME_FORMAT)
        self.to_date_obj = datetime.datetime.strptime(self.to_date, DEFAULT_SERVER_DATETIME_FORMAT)
